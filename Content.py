...
treasureBoxText = [
    '清理arp缓存表', '重置winsock', '重新加载dll', 'IE安全警报关闭', '修复windows installer', '组策略启动修复', '修复dns并重新分配ip', '解除CMD禁用', '解除注册表禁用', '解除防火墙禁用（win10）', '路由表重置', '重置系统应用（win10）', '清理桌面图标缓存（win10）'
]
...
def ErrorOptimizationClean():
    ...
    system(' reg add "HKEY_CURRENT_USER\\Control Panel\\Desktop" /v WaitToKillAppTimeout /t REG_SZ /d 20000 /f')
    system(' reg add "HKEY_CURRENT_USER\\Control Panel\\Desktop" /v HungAppTimeout /t REG_SZ /d 5000 /f')
    system(' reg add "HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control" /v WaitToKillServiceTimeout /t REG_SZ /d 12000 /f')
...
treasureBoxFunction = [
    ARPClean, WinsockReset, DllReload, NoWarn, FixMsiServer, FixGpedit, DnsFlush, EnableCMD, EnableREG, EnableFireWall, RouteClean, Win10APPReset, DesktopCacheClean
]